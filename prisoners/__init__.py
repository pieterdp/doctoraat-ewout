    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        for selected_match in selected_matches:
            ...
            for prisoner in PrisonersCompare.query.filter(PrisonersCompare.id_gedetineerde == selected_match).all():
                ...
                prisoner.has_been_checked = True
                db.session.commit()
            for prisoner in PrisonersCompare.query.filter(PrisonersCompare.c_id_gedetineerde == selected_match).all():
                # All prisoners that are compared to this match
                prisoner.has_been_checked = True
                db.session.commit()
            for prisoner in PrisonersCompare.query.filter(PrisonersCompare.c_id_gedetineerde == prisoner_id).all():
